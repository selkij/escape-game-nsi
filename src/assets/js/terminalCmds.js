 ...
 * @param {*} arg Argument de la commande (ex: '-class').
 * @param {*} type Type de composants à énumérer.
 */
 ...
 * @param {*} disk Disque à rendre accessible
 * @param {*} path Chemin final du disque.
 */
  ...
  if (window.commandPrefixText !== "admin@203.45.67.89: ") {
    ...
    switch (disk) {
      ...
      case "/dev/sde1":
        if (path === "/mnt/sde1") {
          setTimeout(
            () => addCommandHistory(`Montage réussi de /dev/sda à ${path}`),
            1300
          );
          window.knownDirectories.push(path);
        } else {
